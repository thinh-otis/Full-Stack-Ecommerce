...
function Header() {
  ...
  useEffect(() => {
    // Lắng nghe sự thay đổi của user từ Redux và cập nhật icon
    if (user) {
  ...
  const handleLogout = async() => {
    ...
    if(data.success){
      ...
      localStorage.removeItem('userDetails'); // Xóa khỏi localStorage khi logout
      navigate("/")
    }
    ...
    <header className="h-16 shadow-md bg-white fixed w-full z-40">
      <div className="h-full container mx-auto flex items-center px-4 justify-between">
        ...
        <div className="hidden lg:flex items-center w-full justify-between max-w-sm border rounded-full focus-within:shadow pl-2">
          <input
            type="text" placeholder="search product here..." className="w-full outline-none" onChange={handleSearch} value={search} />
          <div className="text-lg min-w-[50px] h-8 bg-red-600 flex items-center justify-center rounded-r-full text-white ">
            <GrSearch className="hover:scale-125 transition-all" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-7">
          ...
          <div className="relative flex justify-center">
            ...
            {
              menuDisplay && (
                <div className="absolute bg-white bottom-0 top-11 h-fit p-2 shadow-lg rouded">
                  <nav>
                    {
